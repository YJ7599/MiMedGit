...
Sobel_Test = function(data.na, mediator.list, treatment, outcome) {
  ...
  Sobel.result[["P-value"]] <- Sobel.result[["P-value"]]
...
Forestplot_Sobel = function(regression.result, sobel.result, is.tree, mult.test.cor = TRUE) {
  ...
  Med_estimate <- rep(round(Med_mean, 3), each = 2)
  Out_estimate <- rep(round(Out_mean, 3), each = 2)
  
  P_value <- p.value.0.1(rep(sobel.result$`P-value`, each=2))
  
  if (mult.test.cor){
    Q_value <- p.adjust(sobel.result$`P-value`, "BH")
    Q_value <- p.value.0.1(rep(Q_value, each=2))
  }
  ...
  if (mult.test.cor){
    ...
    output_base_data <- tibble(mean = Mean,
                               ...
                               med_estimate = as.character(Med_estimate),
                               out_estimate = as.character(Out_estimate))
  ...
  if (mult.test.cor){
    output_df %>% 
      forestplot(labeltext = c(alpha_div, p_value, q_value, med_estimate, out_estimate),
                 is.summary = summary,
                 xlab = "95% Confidence Interval",
...
Forestplot_Hayes <- function(hayes.result, result.type = c("Indirect", "Direct"), is.tree, mult.test.cor) {
  ...
  if (result.type == "Indirect") {
    ...
    Estimate <- round(Mean, 3)
    P_value <- p.value.0.1(P_value)
    
    if (mult.test.cor){
      Q_value = p.value.0.1(p.adjust(hayes.result[[2]]$p, "BH"))
    }
    ...
    if (mult.test.cor){
      ...
      output_df %>% 
        forestplot(labeltext = c(alpha_div, estimate, p_value, q_value),
                   is.summary = summary,
                   xlab = "95% Confidence Interval",
                   txt_gp = fpTxtGp(title = gpar(cex=1.35),
                                    ticks = gpar(cex=0.75),
...
Forestplot_DACT = function(regression.result, DACT.result, is.tree, mult.test.cor) {
  ...
  P_value <- p.value.0.1(rep(DACT.result$`P-value`, each=2))
  
  if (mult.test.cor) {
    Q_value <- p.adjust(DACT.result$`P-value`, "BH")
    Q_value <- p.value.0.1(rep(Q_value, each = 2))
  }
...
Forestplot = function(med.out, is.tree,
                      ...
                      effect = c("ACME (Overall)", "ADE", "Total Effect",
                                 "ACME (Control)", "ACME (Treated)"), mult.test.cor = TRUE) {
  ...
  if (mult.test.cor){
    Q_value <- ifelse(abs(round(Q_value, 3)) == 0, "<.001", sprintf("%.3f", Q_value))
