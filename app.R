      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (input$choose_taxa == "Imai Method"){
          ...
          if (binary_cont(chooseData$sam.dat[,input$treat_taxa]) == "binary"){
            ...
            med_result <<- tryCatch(mediation.taxon.total(chooseData$sam.dat, taxa, taxa_unique, input$treat_taxa, covariate_taxa, input$outcome_taxa, interaction_taxa, taxa_reg_type, taxa_sim_type, boot.method.po = boot.iterations, n.sim = 500, inc = include), 
                                    error = function(e) {
                                      message("No outcome is available!")
                                      showModal(modalDialog(div("No outcome is available!")))
                                      return(NULL)})
