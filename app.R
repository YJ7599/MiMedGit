...
list.of.packages <- c('shiny', 'rmarkdown', 'seqinr', 'shinydashboard', 'dashboardthemes', 'tidyverse', 'plotly', 'shinyWidgets', 'shinyjs', 'googleVis', 'xtable', 
                      ...
                      'entropart', 'dirmult', 'robustbase', 'erer', 'BiasedUrn', 'CompQuadForm', 'robCompositions', 'fresh', 
...
if(!require('GLMMMiRKAT')) remotes::install_github('https://github.com/hk1785/GLMM-MiRKAT')
...

{
  ...
  HOME_COMMENT4 = p(strong("Reference:"), " Jang, H., Park, S., Koh, H. (2023) Comprehensive microbiome causal mediation analysis using MiMed on user-friendly web interfaces. Biology Methods and Protocols, 8(1), bpad023.", style = "font-size:13pt")
  ...
  DATA_TRANSFORM_COMMENT = p("Normalize the data into three different formats (1) CLR (centered log ratio) (Aitchison, 1982), (2) arcsine-root and (3) proportion for each taxonomic rank (phylum, class, order, family, genus, species).")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(id = "side_menu",
                ...
                menuItem("Community-level Analysis",  
                ...
                menuItem("Taxonomy-level Analysis", tabName = "TAX_Trans", 
...
server <- function(input, output, session) {
          ...
          observeEvent(input$interaction,{
            ...
            if (input$outvar %in% chooseData$out_vars) {
              ...
              if (is.results$outResult == "Binary") {
                ...
                output$mul.cor.alpha <- renderUI({
                  tagList(
                    p(" ", style = "margin-top: 25px;"),
                    h4(strong("Multiple testing correction?", style = "color:black")),
                    p(" ", style = "margin-bottom: +15px;"),
                    prettyRadioButtons("bh.pval.alpha",label = NULL, icon = icon("check"),
                                       animation = "jelly", c("Yes (Default)", "No"), 
                                       selected = "Yes (Default)", width = '70%')
                  )
                })
        ...
        if (is.results$beta.treatResult == "Binary") {
          ...
          observeEvent(input$beta.outvar_cross,{
            ...
            if (input$beta.outvar_cross %in% chooseData$out_vars) {
              ...
              output$mul.cor.beta <- renderUI({
                tagList(
                  p(" ", style = "margin-top: 25px;"),
                  h4(strong("Multiple testing correction?", style = "color:black")),
                  p(" ", style = "margin-bottom: +15px;"),
                  prettyRadioButtons("bh.pval.beta",label = NULL, icon = icon("check"),
                                     animation = "jelly", c("Yes (Default)", "No"), 
                                     selected = "Yes (Default)", width = '70%')
                )
              })
    ...
    output$taxa_treat <- renderUI({
      tagList(
        prettyRadioButtons("dataType_taxa", label = h4(strong("Data format", style = "color:black; font-size:12.7pt")), animation = "jelly", icon = icon("check"),
                           c("CLR (Default)", "Arcsine-root", "Proportion"), selected = "CLR (Default)",width = '60%'), 
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if(!is.null(tax.tab)){
          if (input$kingdom != "all") {
            ...
            print(tax.tab[,1])
            print("kingdom print now")
            print(input$kingdom)
            print("kingdom print done ")
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        output$datTransDownload <- renderUI({
          tagList(
            box(title = strong("Download Data", style = "color:white"), width = NULL, status = "primary", solidHeader = TRUE,
                ...
                h5("Arcsine-root"),
                downloadButton("taxadataArc", "Download", width = '50%', style = "background-color: red3"), br(), 
                h5("Proportion"),
        ...
        output$taxadataProp <- downloadHandler(
          filename = function() {
            paste("Proportion.Transformed.Data.zip")
          },
          content = function(clr.file) {
            temp <- setwd(tempdir())
            on.exit(setwd(temp))
  ...
  observeEvent(input$runbtn_alpha,{
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (input$chooseMethod == "Imai Method (Default)") {
          ...
          output$forestplot_ACME1 <- renderPlot({
            tryCatch(Forestplot(Med.out, Is.tree, Outcome.type, Treatment.type, Interaction, effect = "ACME (Overall)", bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
          ...
          output$forestplot_ACME2 <- renderPlot({
            tryCatch(Forestplot(Med.out, Is.tree, Outcome.type, Treatment.type, Interaction, effect = "ACME (Control)", bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
          ...
          output$forestplot_ACME3 <- renderPlot({
            tryCatch(Forestplot(Med.out, Is.tree, Outcome.type, Treatment.type, Interaction, effect = "ACME (Treated)", bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
          ...
          output$forestplot_ADE <- renderPlot({
            tryCatch(Forestplot(Med.out, Is.tree, Outcome.type, Treatment.type, Interaction, effect = "ADE", bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
          ...
          output$forestplot_TOTAL <- renderPlot({
            tryCatch(Forestplot(Med.out, Is.tree, Outcome.type, Treatment.type, Interaction, effect = "Total Effect", bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
            ...
            fluidRow(style = "position:relative",
                     tabBox(width = 12, title = strong("Forest Plot", style = "color:black"),
                            tabPanel("Sobel Test", align = "center",
                                     plotOutput("Sobel_forestplot", height = 500, width = 800))))
          })
          
          check_to_see <<- Regression_result 
          check_to_see_2 <<- Sobel_result 
          
          output$Sobel_forestplot <- renderPlot({
            tryCatch(Forestplot_Sobel(Regression_result, Sobel_result, Is.tree, bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
        ...
        # (3) Preacher and Hayes Bootstrap ---------------------------------------------
          ...
          output$Hayes_forestplot_indirect <- renderPlot({
            tryCatch(Forestplot_Hayes(Hayes_result, "Indirect", Is.tree, bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
          ...
          output$Hayes_forestplot_direct <- renderPlot({
            tryCatch(Forestplot_Hayes(Hayes_result, "Direct", Is.tree, bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
        ...
        # (4) Divided-Aggregate Composite-Null Test ---------------------------------------------
          ...
          output$DACT_forestplot <- renderPlot({
            tryCatch(Forestplot_DACT(Regression_result, DACT_result, Is.tree, bh.correction), 
                     error = function(e) {
                       message("No outcome is available!")
                       showModal(modalDialog(div("No outcome is available!")))
                       return(NULL)})
    ...
    if (bh.correction){
      ref_string = REFERENCE_CHECK_M(method_name = isolate(input$chooseMethod), FDR = "Yes")
    } else {
  ...
  observeEvent(input$beta.runbtn,{
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (Is.tree == "withTree") {
          ...
          check_result_1 <<- Med.test.out 
          
          
          output$beta_graph_plot1 <- renderPlot({
            
            if (Treatment.type == "Binary" & Outcome.type == "Binary") {
              try(MedTest.bin.bin.plot1(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            }
            else if (Treatment.type == "Continuous" & Outcome.type == "Binary") {
              try(MedTest.con.bin.plot1(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            }
            else if (Treatment.type == "Binary" & Outcome.type == "Continuous") {
              try(MedTest.bin.con.plot1(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            } 
            else {
              try(MedTest.con.con.plot1(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            }
          ...
          output$beta_graph_plot2 <- renderPlot({
            ...
            if (Treatment.type == "Binary" & Outcome.type == "Binary") {
              try(MedTest.bin.bin.plot2(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
              
            }
            else if (Treatment.type == "Continuous" & Outcome.type == "Binary") {
              try(MedTest.con.bin.plot2(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), 
                  silent = TRUE) 
            }
            else if (Treatment.type == "Binary" & Outcome.type == "Continuous") {
              try(MedTest.bin.con.plot2(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            }
            else {
              try(MedTest.con.con.plot2(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
          ...
          output$beta_graph_plot <- renderPlot({
            ...
            if (Treatment.type == "Binary" & Outcome.type == "Binary") {
              try(MedTest.bin.bin.plot(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            }
            else if (Treatment.type == "Continuous" & Outcome.type == "Binary") {
              try(MedTest.con.bin.plot(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            }
            else if (Treatment.type == "Binary" & Outcome.type == "Continuous") {
              try(MedTest.bin.con.plot(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            } 
            else {
              try(MedTest.con.con.plot(Med.test.out, beta.Treatvar.out, beta.Outvar.out, bh.correc.beta), silent = TRUE) 
            }
          })
    ...
    if (bh.correc.beta){
      ref_string = REFERENCE_CHECK_M(method_name = isolate(input$beta.chooseMethod), FDR = "Yes")
    } else {
  ...
  observeEvent(input$runbtn_taxa, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        taxa_0530 <<- chooseData$taxa.out
        type <<- taxa.types$dataType
        ...
        if (input$choose_taxa == "Imai Method"){
          ...
          if (binary_cont(chooseData$sam.dat[,input$treat_taxa]) == "binary"){
            ...
            med_result <<- tryCatch(mediation.taxon.total(chooseData$sam.dat, taxa, taxa_unique, input$treat_taxa, covariate_taxa, input$outcome_taxa, interaction_taxa, taxa_reg_type, taxa_sim_type, boot.method.po = boot.iterations, n.sim = 2000, inc = include), 
                                    error = function(e) {
                                      message("No outcome is available!")
                                      showModal(modalDialog(div("No outcome is available!")))
                                      return(NULL)})
                                                       ...
                                                       tagList(
                                                         ...
                                                         box(id = "chat_overall", title = strong("Ask ChatGPT", style = "color:white", side = "left"), width = NULL, 
                                                             ...
                                                             column(width = 6.5, 
                                                                    ...
                                                                    p("You can visit", a("https://platform.openai.com/account/api-keys", href = "https://platform.openai.com/account/api-keys", target = "blank"), " to download your private ChatGPT API Key.",  style = "font-size:10pt"), 
            ...
            observeEvent(input$chatgpt_rank_overall, {
              ...
              output$other_opt_chat_overall_2 <- renderUI({tagList(
                ...
                actionButton("runbtn_chat_ove", strong("Ask!"), class = "btn-info",
                             style="color: #000000; background-color: #FFFFFF; border-color: #2C3E50"),
                p(strong("Warning:"), " ChatGPT is not always right, and the results from ChatGPT need to be used with caution.") 
              )})
            }) 
            ...
            observeEvent(input$runbtn_chat_ove, {
              withProgress(message = 'Asking ChatGPT', value = 0, {
                ...
                output$chat_vis_overall <- renderUI({
                  tagList(box(title = NULL, width = "60%", strong(paste0(paste("What is known about", input$rename_taxon_ove, "on", input$rename_1_ove, "and", input$rename_2_ove), "?"))),
                          p(" ", style = "margin-bottom: -15px;"),
                          box(title = NULL, width = "60%",
                              p(chat_result_ove)),
                          
                          p(" ", style = "margin-bottom: +15px;"),  
                          box(title = NULL, width = "60%", strong("Reference Search Results"), 
                              p(" ", style = "margin-bottom: +15px;"),
                              p("Google Scholar"), 
                              p(" ", style = "margin-bottom: +10px;"),
                              p(tags$a(href = paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon_ove), "+", gsub(" ", "+",input$rename_1_ove), "+", gsub(" ", "+",input$rename_2_ove), "&btnG="), paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon_ove), "+", gsub(" ", "+",input$rename_1_ove), "+", gsub(" ", "+",input$rename_2_ove), "&btnG="))), 
                              
                              p("PubMed"), 
                              p(" ", style =  "margin-bottom: +10px;"), 
                              p(tags$a(href = paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon_ove), "+", gsub(" ", "+",input$rename_1_ove), "+",  gsub(" ", "+",input$rename_2_ove)), paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon_ove), "+",  gsub(" ", "+",input$rename_1_ove), "+", gsub(" ", "+",input$rename_2_ove))))), 
                  )  
                })
              }) 
            })
          ...
          observeEvent(input$chatgpt_rank_acme, {
            ...
            output$other_opt_chat_acme_2 <- renderUI({tagList(
              ...
              p("You can rename it using a human language, for instance, deleting possible codes, special symbols or numbers (e.g., from ‘Erysipelotrichaceae_[G-1]‘ to ‘Erysipelotrichaceae‘)", style = "font-size:10pt"),
              textInput("rename_1_acme", label = paste0("Rename the treatment variable"), value = input$treat_taxa, width = "55%"),
              p("You can rename the treatment variable using a human language (e.g., from ‘ecig_status’ to ‘e-cigarrette’).", style = "font-size:10pt"),
              textInput("rename_2_acme", label = paste0("Rename the outcome variable"), value = input$outcome_taxa, width = "55%"),
              p("You can rename the outcome variable using a human language (e.g., from ‘gingival_inflammation’ to ‘gingival inflammation’).", style = "font-size:10pt"),
          ...
          observeEvent(input$runbtn_chat_acme, {
            withProgress(message = 'Asking ChatGPT', value = 0, {
              ...
              print("gpt run acme ")
              print(input$api_gpt_acme)
              print("gpt done acme")
              a_check <<- input$api_gpt_acme
              b_check <<- input$rename_taxon_acme 
              c_check <<- input$rename_1_acme
              d_check <<- input$rename_2_acme
              ...
              output$chat_vis_acme <- renderUI({
                tagList(box(title = NULL, width = "60%", strong(paste0(paste("What is known about", input$rename_taxon_acme, "on", input$rename_1_acme, "and", input$rename_2_acme), "?"))),
                        p(" ", style = "margin-bottom: -15px;"),
                        box(title = NULL, width = "60%",
                            p(chat_result_acme)),
                        ...
                        box(title = NULL, width = "60%", strong("Reference Search Results"), 
                            ...
                            p(tags$a(href = paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon_acme), "+", gsub(" ", "+", input$rename_1_acme), "+", gsub(" ", "+", input$rename_2_acme), "&btnG="), paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon_acme), "+", gsub(" ", "+", input$rename_1_acme), "+", gsub(" ", "+", input$rename_2_acme), "&btnG="))), 
                            
                            p("PubMed"), 
                            p(" ", style =  "margin-bottom: +10px;"), 
                            p(tags$a(href = paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon_acme), "+", gsub(" ", "+",input$rename_1_acme), "+",  gsub(" ", "+",input$rename_2_acme)), paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon_acme), "+",  gsub(" ", "+",input$rename_1_acme), "+", gsub(" ", "+",input$rename_2_acme))))), 
          ...
          result_sobel_med_ori <<- tryCatch(result_med_out(chooseData$sam.dat, taxa, taxa_unique, input$treat_taxa, input$outcome_taxa, "med", interac = FALSE), 
                                        error = function(e) {  
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
          result_sobel_med <<- tryCatch(med_result_concate_sobel(taxa, result_sobel_med_ori, taxa_unique, include), 
                                        error = function(e) {   nkl[[po[]]]
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
            
          #here 
          result_sobel_med <<- tryCatch(q_convert_tax_sobel(result_sobel_med, include), 
                                        error = function(e) {   nkl[[po[]]]
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
       
          result_sobel_out_ori <<- tryCatch(result_med_out(chooseData$sam.dat, taxa, taxa_unique, input$treat_taxa, input$outcome_taxa, "out", interac = FALSE), 
                                        error = function(e) {  
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})

          result_sobel_out <<- tryCatch(med_result_concate_sobel(taxa, result_sobel_out_ori, taxa_unique, include), 
                                        error = function(e) {  
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
          
          result_sobel_out <<- tryCatch(q_convert_tax_sobel(result_sobel_out, include), 
                                        error = function(e) {   nkl[[po[]]]
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
          
          med_result_sobel_ori <<- tryCatch(tax.sobel(chooseData$sam.dat, taxa, input$treat_taxa, input$outcome_taxa, inc = include), 
                                        error = function(e) {  
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
          
          med_result_sobel <<- tryCatch(med_result_concate_sobel(taxa, med_result_sobel_ori, taxa_unique, include), 
                                        error = function(e) {  
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})

          med_result_sobel <<- tryCatch(q_convert_tax_sobel(med_result_sobel, include), 
                                        error = function(e) {  
                                          message("No outcome is available!")
                                          showModal(modalDialog(div("No outcome is available!")))
                                          return(NULL)})
          ...
          observeEvent(input$runbtn_chat_sobel, {
            withProgress(message = 'Asking ChatGPT', value = 0, {
              ...
              output$chat_vis_sobel <- renderUI({
                tagList(box(title = NULL, width = "60%", strong(paste0(paste("What is known about", input$rename_taxon_sobel, "on", input$rename_1_sobel, "and",input$rename_2_sobel), "?"))),
                        p(" ", style = "margin-bottom: -15px;"),
                        box(title = NULL, width = "60%",
                            p(chat_result_sobel)),
                        ...
                        box(title = NULL, width = "60%", strong("Reference Search Results"), 
                            ...
                            p(tags$a(href = paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon_sobel), "+", gsub(" ", "+",input$rename_1_sobel), "+", gsub(" ", "+",input$rename_2_sobel), "&btnG="), paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon_sobel), "+", gsub(" ", "+",input$rename_1_sobel), "+", gsub(" ", "+",input$rename_2_sobel), "&btnG="))), 
                            p("PubMed"), 
                            p(" ", style =  "margin-bottom: +10px;"), 
                            
                            p(tags$a(href = paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon_sobel), "+", gsub(" ", "+",input$rename_1_sobel), "+",  gsub(" ", "+",input$rename_2_sobel)), paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon_sobel), "+",  gsub(" ", "+",input$rename_1_sobel), "+", gsub(" ", "+",input$rename_2_sobel)))))
                )  
              })
            }) 
          })
          ...
          if (length(table(chooseData$sam.dat[, input$outcome_taxa])) != 2){
            ...
            dat_1 <<- chooseData$sam.dat
            dat_2 <<- taxa 
            dat_3 <<- exposure 
            dat_4 <<- outcome 
            dat_5 <<- covariates 

            result_dact_ind <<- tryCatch(result_med_out_dact(chooseData$sam.dat, taxa, taxa_unique, exposure, outcome, covariates, reg = "linear", interac = FALSE), error = function(e) {  
              message("No outcome is available!")
              showModal(modalDialog(div("No outcome is available!")))
              return(NULL)})
            ...
            result_dact_ind <<- tryCatch(result_med_out_dact(chooseData$sam.dat, taxa, taxa_unique, exposure, outcome, covariates, reg = "logistic", interac = FALSE), 
                                         error = function(e) {  
              message("No outcome is available!")
              showModal(modalDialog(div("No outcome is available!")))
              return(NULL)})
                ...
                output$chat_vis <- renderUI({
                  tagList(box(title = NULL, width = "60%", strong(paste0(paste("What is known about", input$rename_taxon, "on", input$rename_1 , "and",input$rename_2), "?"))),
                          ...
                          box(title = NULL, width = "60%", strong("Reference Search Results"), 
                              ...
                              p(tags$a(href = paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon), "+", gsub(" ", "+",input$rename_1), "+", gsub(" ", "+",input$rename_2), "&btnG="), paste0("https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&q=", gsub(" ", "+", input$rename_taxon), "+", gsub(" ", "+",input$rename_1), "+", gsub(" ", "+",input$rename_2), "&btnG="))), 
                              
                              p("PubMed"), 
                              p(" ", style =  "margin-bottom: +10px;"), 
                              p(tags$a(href = paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon), "+", gsub(" ", "+",input$rename_1), "+",  gsub(" ", "+",input$rename_2)), paste0("https://pubmed.ncbi.nlm.nih.gov/?term=", gsub(" ", "+",input$rename_taxon), "+",  gsub(" ", "+",input$rename_1), "+", gsub(" ", "+",input$rename_2))))
                          )
                  )  
                })
              }) 
